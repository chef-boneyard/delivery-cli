...
use utils::{copy_recursive, file_needs_updated, read_file, walk_tree_for_path};
...
impl Default for DeliveryConfig {
    fn default() -> Self {
        ...
        build_cookbook.insert("name".to_string(), "build_cookbook".to_string());
        build_cookbook.insert("path".to_string(), ".delivery/build_cookbook".to_string());
...
impl DeliveryConfig {
        ...
        P: AsRef<Path>,
    {
        match walk_tree_for_path(proj_path, ".delivery/config.json") {
            ...
            None => Err(DeliveryError {
                kind: Kind::MissingProjectConfig,
                detail: None,
            }),
        }
    }
        ...
        P: AsRef<Path> + Debug,
    {
        ...
        let json: DeliveryConfig = serde_json::from_str(&config_json).or_else(|e_v2| {
            debug!(
                "Unable to parse DeliveryConfig: {}\nV2 Error: {}",
                config_json, e_v2
            );
    ...
    pub fn load_raw_config(p_path: &PathBuf) -> DeliveryResult<SerdeJson> {
        ...
        debug!(
            "Loading RAW config.json into memory from path: {}",
            p_path.display()
        );
        let config_json = read_file(&DeliveryConfig::find_config_file(p_path)?)?;
...
impl DeliveryConfigV1 {
    ...
    fn convert_to_v2(self) -> DeliveryResult<DeliveryConfig> {
        ...
        if self.build_cookbook.contains("/") {
            ...
            build_cookbook.insert(
                String::from("name"),
                cookbook_name.to_string_lossy().into_owned(),
            );
            build_cookbook.insert(
                String::from("path"),
                cookbook_path.to_string_lossy().into_owned(),
            );
        } else {
            // A build_cookbook name, load it from the `server`
            build_cookbook.insert(String::from("name"), self.build_cookbook);
    ...
    mod v2 {
        ...
        mod job_dispatch {
            ...
            fn complex_config() {
                ...
                assert!(
                    complex.filters.clone().unwrap().get("unit").unwrap()[0]
                        .get("platform_family")
                        .is_some()
                );
                // Extract the platform_version content of the filter1 for the syntax phase
                assert_eq!(
                    complex.filters.clone().unwrap().get("syntax").unwrap()[0]
                ...
                assert_eq!(
                    complex.filters.clone().unwrap().get("publish").unwrap()[1]
                        .get("platform_family")
                        .unwrap()[0],
                    String::from("rhel")
                );
